...
def test_description_roundtrip_punctuation():
    ...
    s, e, hook, title, filler, description, x_list, c_list, k_list, errors = _parse_fields(
...
def test_only_t_parses():
    s, e, hook, title, filler, description, x_list, c_list, k_list, errors = _parse_fields(" t: Только заголовок")
    assert errors == []
...
def test_only_d_parses():
    s, e, hook, title, filler, description, x_list, c_list, k_list, errors = _parse_fields(" d: Только описание.")
    assert errors == []
    assert title is None
...
def test_neither_t_nor_d_is_fine():
    s, e, hook, title, filler, description, x_list, c_list, k_list, errors = _parse_fields(" s:2 e:5")
    assert errors == []
    assert title is None
    assert description is None
    assert s == 2 and e == 5
