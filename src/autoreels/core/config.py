...
class AudioProcessing(BaseModel):
    ...
    # Кроссфейд звука на КАЖДОМ внутреннем стыке многосегментного клипа (убирает щелчок на
    # склейке). Только для клипов с >1 сегментом; одиночный span не затрагивается. Видео
    # склеивается встык, аудио — с этим кроссфейдом (укорачивает звук на ~длину_фейда·(N−1),
    # доли кадра — синхронность субтитров сверяется на глаз). 0 → встык, без кроссфейда.
    audio_crossfade_sec: float = 0.03
