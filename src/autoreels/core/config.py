...
class AudioProcessing(BaseModel):
    ...
    # Аудио-фейд в САМОМ конце клипа (только звук, БЕЗ видео-фейда в чёрное). Ложится на
    # tail_pad-«воздух» после последнего слова — граничный клип не обрывается резко и не звучит
    # начало следующей фразы, случайно попавшее в хвост. Включён по умолчанию для всех клипов.
    # Отдельно от 10-мс edge-фейдов на стыках сегментов.
    audio_tail_fade_sec: float = 0.35
