...
def dedup_reels(reels: list[Reel], threshold: float) -> list[Reel]:
    ...
    Сохраняет порядок входного списка. При пересечении > threshold оставляет
    первый встреченный (по позиции в списке), дубль выбрасывает.
    Используется после склейки R0-результатов нескольких чанков.
    """
    kept: list[Reel] = []
    for r in reels:
        duplicate = False
