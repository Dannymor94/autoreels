...
| `{{max_duration}}` | пресет длины | 90 |

System-промпт остаётся байт-в-байт стабильным при фиксированных значениях. Менять пресет = менять промпт (это ожидаемо и редко).
